...
@app.post("/api/orders")
def create_order(
    payload: OrderCreatePayload = Body(...), 
    db: Session = Depends(get_db),
    current_user: User = Depends(user_routes.get_current_active_user)
):
...
def get_batches_summary(
    ...
    """
    Get summary of batches, optionally filtered by date range
    """
    stage_sequence = _stage_sequence()
    query = db.query(BatchTracking)

    if start_date:
        query = query.filter(BatchTracking.created_at >= start_date)
    if end_date:
        query = query.filter(BatchTracking.created_at <= end_date)
    ...
    try:
        ...
        if initial_stage:
            try:
                ...
                # If inventory update fails, rollback batch creation
                db.rollback()
